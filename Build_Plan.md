...
## Phase 1A: Python backend take-home (~90 min, before the session)

Build a FastAPI service that powers case review.
...
- Python 3.12, FastAPI, Pydantic, and uv-managed dependencies
- Sensible package structure (routers, services, models, storage, your call on the split)
- Input validation with meaningful error responses (400, 404)
- At least 3 unit tests covering merge logic edge cases
- Runnable locally with `uv run uvicorn backend.main:app --reload --port 8000`
...
1. **`Dockerfile`** for the FastAPI service. Multi-stage build, reasonable image size, non-root runtime user, pinned base image versions.
...
You'll build the reviewer's screen, calling your own Python backend from Phase 1.

### Setup before the call

- Your Python backend running locally on a known port (use your `ops/run.sh start`)
...
- Phase 1A (Python backend): ~90 min
...
- `case_v1.json`: bootstrap data for your Python backend
